...
export class AuthDAO implements IAuthDAO {
  ...
  async createUser(payload: Login): Promise<any> {
   ...
   async respondNewPassword(p: ChallengeNewPassword): Promise<LoginResult> {
    ...
    const resp = await this.client.send(new RespondToAuthChallengeCommand(input));
    if (resp.ChallengeName) {
      // Raro, pero podría encadenar a MFA
      return { ok: false, data: { challenge: resp.ChallengeName, session: resp.Session, parameters: resp.ChallengeParameters } };
    }
    const a = resp.AuthenticationResult!;
    return { ok: true, data: {
      accessToken: a.AccessToken!, idToken: a.IdToken!, refreshToken: a.RefreshToken,
      expiresIn: a.ExpiresIn!, tokenType: a.TokenType!
    }};
  }
  ...
  async respondMfa(p: ChallengeMfa): Promise<LoginResult> {
    ...
    const input: RespondToAuthChallengeCommandInput = {
      ...
      ChallengeResponses: {
        ...
        ...(challenge === 'SOFTWARE_TOKEN_MFA' && p.code ? { SOFTWARE_TOKEN_MFA_CODE: p.code } : {}),
      },
    };
    const resp = await this.client.send(new RespondToAuthChallengeCommand(input));
    if (resp.ChallengeName) {
